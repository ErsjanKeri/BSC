...
Repository: \texttt{github.com/ErsjanKeri/llama.cpp}, branch \texttt{expert-pipeline-fused}, head \texttt{d7e480f4}. The fork carries twenty-four commits on top of upstream llama.cpp \cite{ggerganov2023llamacpp} plus a working-tree diff covering the async-projection-overlap and async-experts pipelines (sixteen tracked source files in the \texttt{llama-io-uring}, \texttt{llama-moe-pipeline}, and \texttt{llama-graph} translation units, the \texttt{arg} and \texttt{common} units in \texttt{common/}, and a small change in \texttt{include/llama.h}). The thesis state is HEAD \texttt{d7e480f4} plus that working-tree diff; line numbers cited below describe the working tree.
...
Step 1 clones the fork and checks out the canonical commit. The thesis state is HEAD \texttt{d7e480f4} plus the working-tree diff documented in \autoref{sec:art-llamacpp}; if a tagged thesis commit has been produced by the time of reproduction, check out that tag instead.

\begin{lstlisting}[basicstyle=\ttfamily\small,breaklines=true]
# 1. Clone the fork and check out the canonical commit.
git clone https://github.com/ErsjanKeri/llama.cpp ~/llama.cpp
cd ~/llama.cpp && git checkout d7e480f4
# Apply the working-tree diff if reproducing the dirty state:
#   git apply path/to/working-tree.diff
...
The CSVs should match the canonical numbers within the cited CV bounds: mean CV across the 168-run sweep is 0.23\%, with the noisiest cell at 2.29\% (20B 8~GiB async-experts). If they do not, the most likely cause is a working-tree drift in the fork: re-check the commit hash and the working-tree diff against \autoref{sec:art-llamacpp}. The next most likely cause is page-cache contamination between iterations on a host where the harness was not run with \texttt{sudo}; \texttt{drop\_caches} requires root.
