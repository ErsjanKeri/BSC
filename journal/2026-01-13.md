...
# Build map: tensor name → addresses seen in trace
trace_addrs = {}
...
for trace_name, trace_addr_list in list(trace_addrs.items())[:20]:
    ...
    if dot_addr is None:
        print(f"⚠️  '{trace_name}' in trace but NOT in DOT file")
        continue

    if dot_addr in trace_addr_list:
        print(f"✓  '{trace_name}' - addresses MATCH: {dot_addr}")
    else:
        print(f"✗  '{trace_name}' - addresses MISMATCH:")
...
3. Build separate indices:
   - `nameToGraphNode`: Tensor name → Graph node
   - `nameToTraces`: Tensor name → Trace entries
   - `nameToMemory`: Tensor name → Memory map entry (GGUF weights only)
...
**At 100x zoom:**
- 8KB tensors: ~0.78px → 2px min (clearly visible)
...
return `rgb(${r}, 0, 0)`;  // Dark red → Bright red
```
...
- **Full-screen mode:** Multi-line verbose view (py-3, 60-80px per entry)
  ...
  - Result: ~850 entries × 70px = 59,500px tall, causing performance issues
...
```
🔗 | # | Time | Operation | Destination | Sources | Lyr | Mem | Size
●  | 0 | 0.00 | GET_ROWS  | inp_embd    | token_embd.weight +1 | - | DSK | 125MB
```

**Columns:**
1. **🔗** (4px): Green ● if correlates with graph, gray ○ if not
2. **#** (10px): Entry ID
...
│ SOURCES (2):                            │
│   [0] token_embd.weight                 │
│       DISK • 125.0MB • offset: 0x7db... │
│   [1] leaf_2                            │
│       BUFFER • 8B • buffer: 0x11e...    │
│                                         │
│ Total input size: 125.0MB               │
└─────────────────────────────────────────┘
```
...
**Data Quality Fixes:**
1. ✅ 1024-byte trace format (128-byte names) - NO MORE TRUNCATION
2. ✅ GGUF offset auto-calculation - ZERO OVERLAPS
3. ✅ F16 model - Simple, clean sizes (2 bytes/element)
4. ✅ Name-based correlation - 100% ACCURATE (589 nodes, 787 entries)

**Layout & Interaction:**
5. ✅ Dynamic 3-view layout (adapts to 1/2/3 visible views)
6. ✅ Drag & drop reordering (header-only, no slider conflicts)
7. ✅ View toggle buttons (Graph ON/OFF, Logs ON/OFF, Heatmap ON/OFF)
8. ✅ Inter-view connectivity (click anywhere → highlights everywhere)

**Heatmap Features:**
9. ✅ Temporal highlighting (synced with timeline)
10. ✅ 2-mode toggle (Total Accumulated vs Current Layer Only)
11. ✅ Zoom control (1x to 500x, makes 8KB tensors visible)
12. ✅ Continuous red gradient (dark → bright based on access count)
13. ✅ Enhanced hover tooltips (all info: name, layer, size, offset, count, timestamps)
14. ✅ Buffer visualization (KV cache, 44MB, status)

**TraceView Features:**
15. ✅ Unified compact design (26px rows, works for all widths)
16. ✅ Destination column added
17. ✅ Sources column shows "first_source +N"
18. ✅ Hover tooltip shows beautiful detailed card (DESTINATION, all SOURCES, sizes, offsets)
19. ✅ Layer=null handled gracefully (shows "-")
20. ✅ Memory badges (DSK/BUF/D+B) color-coded
...
**Before:**
- Full-screen TraceView: 850 entries × 70px = 59,500px tall
- All entries rendered at once (no virtualization)
- Laggy scrolling
- Cluttered, hard to read

**After:**
- Compact TraceView: 850 entries × 26px = 22,100px tall
- **62% height reduction**
...
4. **Interactive Correlation:**
   - Click on graph node → See logs + heatmap regions
   - Click on log entry → See graph + heatmap
   - Click on heatmap → See logs + graph
   - All working via name-based matching

5. **Temporal Analysis:**
   - Scrub timeline → Watch heatmap "heat up"
...
3. Observe: Does heat move through layers sequentially (L0 → L1 → L2...) or randomly?
...
### Final Fixes: C/C++ Compatibility

#### Issue Discovered During Server Deployment

When building llama.cpp on the TUM server (cli-hiwi-02), the compilation failed with errors:

```
tensor_trace.h:40:15: error: expected constructor, destructor, or type conversion before '(' token
   40 | _Static_assert(sizeof(struct SourceTensorInfo) == 160,
```

**Root Cause:** The header file `tensor_trace.h` uses C11's `_Static_assert`, but it's included by C++ source files (`.cpp`). C++ doesn't recognize `_Static_assert` - it uses `static_assert` instead.

**Files affected:**
- `llama.cpp` (C++ file)
- `llama-kv-cache.cpp` (C++ file)
- `llama-model.cpp` (C++ file)

#### Solution

Modified all three static assertions in `tensor_trace.h` to be compatible with both C and C++:
...
**Applied to:**
1. `SourceTensorInfo` assertion (line 40)
2. `TensorAccessLog` assertion (line 73)
3. `BufferEvent` assertion (line 93)

**Result:** llama.cpp now compiles successfully on both local macOS (Clang) and TUM server (GCC), with both C and C++ source files.
...
# Verify
ls -lh build/bin/llama-completion
```
